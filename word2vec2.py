...
if __name__ == '__main__':
  ...
  with tf.Session(graph=graph) as session:
    ...
    W = session.run(normalized_embeddings)
    np.savetxt('wordvec.csv', W, delimiter = ',')

    ckpt = tf.train.get_checkpoint_state('./saved/')
    if ckpt and ckpt.model_checkpoint_path:
      saver.restore(session, tf.train.get_checkpoint_state('./saved/').model_checkpoint_path)
    ...
    for step in xrange(num_steps):
      ...
      if step % 100000 == 0:
        final_embeddings = normalized_embeddings.eval() # Note that this is expensive (~20% slowdown if computed every 500 steps)
        saver.save(session, './saved/wordvec', global_step=step)
        sim = similarity.eval()
